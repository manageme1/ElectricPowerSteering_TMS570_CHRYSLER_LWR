...
FUNC (WdgM_TimeBaseTickType, WDGM_CODE) WdgM_GetTickCount (void) /* PRQA S 1503 */ /* PRQA S 1505 */
{
   ...
   */
  VAR(WdgM_TimeBaseTickType, AUTOMATIC) tick_count;	/* MyCompany Patch: Add local buffer variable */

  #if (WDGM_TIMEBASE_SOURCE == WDGM_INTERNAL_HARDWARE_TICK)
    WdgIf_GetTickCounter (&tick_count);	/* MyCompany Patch: Supply local buffer variable to get hardware tick value */
  #else
    /* (WDGM_TIMEBASE_SOURCE == WDGM_INTERNAL_SOFTWARE_TICK)
       || (WDGM_TIMEBASE_SOURCE == WDGM_EXTERNAL_TICK) */
    tick_count = g_wdgm_tick_count;	/* MyCompany Patch: Set local buffer variable to software tick value */
  #endif /* (WDGM_TIMEBASE_SOURCE == WDGM_INTERNAL_HARDWARE_TICK) */
    return (tick_count);	/* MyCompany Patch: Return local buffer variable */
}
